...
for view in "$@"; do
  ...
  echo "[m2ad-atta-label-only] start view=${view} gpu=${gpu_id} data_root=${data_root} output_root=${output_root} $(date -Is)"
  ...
  "${python_bin}" scripts/run_m2ad_anomalib_rd4ad_btta.py \
    ...
    --atta-oracle-num 1 \
    --atta-query-strategy median_score \
    --selector-mode expblend
  echo "[m2ad-atta-label-only] done view=${view} gpu=${gpu_id} $(date -Is)"
done

echo "[m2ad-atta-label-only] all done gpu=${gpu_id} views=$* $(date -Is)"
