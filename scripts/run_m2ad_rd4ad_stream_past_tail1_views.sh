...
for view in "$@"; do
  ...
  echo "[m2ad-stream-past-tail1] start view=${view} gpu=${gpu_id} data_root=${data_root} output_root=${output_root} $(date -Is)"
  ...
  echo "[m2ad-stream-past-tail1] done view=${view} gpu=${gpu_id} $(date -Is)"
done

echo "[m2ad-stream-past-tail1] all done gpu=${gpu_id} views=$* $(date -Is)"
